...
FOLDER = "D:/tmp/traindata/"+CODE
DIR = "D:/tmp/models/"+(CODE+MODEL)
dir_ ="D:/tmp/traindata/%s"%(CODE+MODEL)
        ...
        for i in range(self.N):
            for j in range(self.N):
                if self.c[i][j]!=0:
                    string+= "%d %d %f\n"%(i+1,j+1,self.c[i][j])
        return string
G2 = SpinGlass2(CONFIG.seed,N)
if MODEL=="SPINS2":
    print(G2)
G = SpinGlass(CONFIG.seed,N)#default is 12
def H(arr):
...
def h5gen(div = 32,runs=1,skip=1):
    ...
    for i in range(div):
        if i%(div//runs)==0:
            print("||",end="")
            arrs = metropolis(N,N,T,iterate = div_multiplier*div//runs*skip+110000,start = 110000)
        batch = [next(arrs) for x in range(length//div*skip)][::skip]
        if MODEL == "POTTS1":
    ...
    f.close()
if RUNS>0:
    h5gen(div = DSIZE,runs=RUNS,skip=CONFIG.skip)
...
def stats_(seed):
    ...
    try:
        ...
        print("This is either zero or infinity")
def res_plt(seed = 55):
...
def main(unused_argv):
    ...
    # Load training data
    k = 24 if (MODEL=="POTTS1" or N>8) else 1
