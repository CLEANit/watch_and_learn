...
###
#   legacy=[a.copy() for a in arrs]
###
   shape=np.shape(arrs[0])
   size=np.prod(shape)
   m= [np.sum(arr)//2+size//2 for arr in arrs]
   ms = [np.zeros([size+1],dtype=np.int32) for a in range(width)]
...
###
#          H_I=sum(Hs(break_apart(legacy[j])))
#          legacy[j][sites[j][0]][sites[j][1]]*=-1
#          H_F=sum(Hs(break_apart(legacy[j])))
#          print(dH,H_F-H_I)
###
          n=np.random.random()
          if dH <=0 or n < C[j]**dH:
...
def errspinsQueue(B,width,steps,seed):
   ...
   res = [x,[],[]]
   t=time.time()
   res_=metropolis(Hs,N,np.asarray(x),steps,keep=0.8)
   for eq in res_ :
       n=np.sum(eq)
       avg = np.sum([abs(i-N**2//2)*eq[i]/(N**2//2) for i in range(len(eq))])/n
       ...
       #print(eq)
       #print(time.time()-t)
   f_avg = np.sum(res[1])/width
   f_st_dv = np.power(np.sum([(i-f_avg)**2 for i in res[1]])/width,0.5)
   print()
   print(f_avg,f_st_dv)
   print(res)
   return res
...
log_file = open(FILENAME.split("EDNN")[0]+
                "/M-%dx%d-%f.log"%(kwargs["N"],kwargs["N"],kwargs["B"]),"w")
